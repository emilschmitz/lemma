...
You optimize **one path only**: `db_extension_runtime/` (Chunk API — Lemma owns filter + agg).
...
`rust_bridge/src/runtime_h1.rs` (+ `db_extension_paths/src/lemma_stream.cpp` when needed).
...
See `research_loop/agents/AGENTS.md`. Optimize **`SESSION_HOT_US`**.
