...
You optimize **one path only**: `db_extension_paths/` — **sidecar `.lemma_cols` copy** into Lemma-owned memory.

Pin/stream/lease e2e moved to `db_extension_lease/`; chunk path is `db_extension_runtime/`.
...
See `research_loop/agents/AGENTS.md`. **Current H1 e2e gap vs DuckDB: primarily agent/kernel**, not missing path scaffolding. Do not push analytical WHERE/SUM back to DuckDB SQL to fake a win.
