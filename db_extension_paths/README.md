...
make -C db_extension_paths extension
# → build/lemma_verus.duckdb_extension

cargo build --release --manifest-path db_extension_paths/rust_bridge/Cargo.toml
cargo test --manifest-path research_loop/agent_primitives/Cargo.toml
```
...
db_extension_paths/check_mem.sh \
  cargo build --release \
  --manifest-path db_extension_paths/rust_bridge/Cargo.toml \
  --bin lemma_pin_h1_smoke

db_extension_paths/check_mem.sh \
  db_extension_paths/rust_bridge/target/release/lemma_pin_h1_smoke \
...
```bash
db_extension_paths/check_mem.sh uv run python db_extension_paths/measure_pin_h1.py
```
...
Lemma↔DuckDB-mem FFI: `research_loop/agent_primitives/src/duckdb_pin.rs`.
