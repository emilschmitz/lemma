...
pub mod _module {
    use crate::_dafny_externs::{ColsNative, NativeAggMap};
    ...
    impl _default {
        ...
        pub fn RunQuery(cols: &Object<ColsNative>) -> Map<(u32, Sequence<DafnyChar>), i64> {
            ...
            while i > 0 {
                i -= 1;
                if cols_ref.EqAtC_REGION_usize(i, "AMERICA") && cols_ref.EqAtS_REGION_usize(i, "AMERICA") && cols_ref.EqAtP_MFGR_usize(i, "MFGR#1") {
                    let mut yr: u32 = cols_ref.GetD_YEAR_usize(i);
                    let mut nation: Sequence<DafnyChar> = cols_ref.GetC_NATION_usize(i);
                    let mut key: (u32, Sequence<DafnyChar>) = (
                            yr,
                            nation.clone()
                        );
                    let mut term: i64 = _default::_native_sub_u64_i64(cols_ref.GetLO_REVENUE_usize(i), cols_ref.GetLO_SUPPLYCOST_usize(i));
...
fn main() {
    ...
    let cols = load_cols_from_tbl("/home/emil/projects/verified-hillclimbing-db/ssb-dbgen/lineorder_flat.tbl", 5000);
    for run in 0..3 {
        let t0 = Instant::now();
