...
```bash
cd /path/to/lemma-db
uv run python verus/research_loop/holdout/gen_data.py
```
...
```bash
cargo build --release --manifest-path verus/research_loop/holdout/bench_holdout/Cargo.toml
```
...
```bash
uv run python verus/research_loop/holdout/run_holdout_bench.py
```
